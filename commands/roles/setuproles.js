...
module.exports = {
    ...
    async execute(message, args) {
        ...
        console.log(`target channel's id is ${target_channel.id}`);
        ...
        let setup_data;
        if (setup_data = await getSetupMessage(guild.id)) {
            // if setup data exists, delete it
            console.log(`setup data is: ${JSON.stringify(setup_data)}`);
        ...
        await addSetup(guild.id, target_channel.id, setup_message.id);
        ...
        console.log(`setupRoles complete`);
    },
};